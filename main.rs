    ...
    let config_task = tokio::spawn(async move {
        ...
        while let Some(config) = config_rx.recv().await {
            ...
            if let Some(value) = config.strip_prefix("TUNCONF:") {
                ...
                let gateway = fields.next().unwrap_or_default();
                crate::log_error!("[КЛИЕНТ] Tunnel IP: {ip}/32 | DNS: {dns} | Gateway: {gateway}");
                config_dispatcher.configure_tun(ip, dns, gateway).await;
            }
